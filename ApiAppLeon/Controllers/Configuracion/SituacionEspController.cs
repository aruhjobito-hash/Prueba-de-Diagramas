...
// Description  : Controlador para control de Situacion Especiales del Trabajador
...
namespace Configuracion.Controllers
{
    ...
    [ApiController]
    public class SituacionEspController : ControllerBase
    {
        private readonly DBContext _dbContext;

        public SituacionEspController(DBContext dbContext)
        {
            _dbContext = dbContext;
        }

        // POST: api/SituacionEsp
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(Respuesta<SituacionEspDBModel>))]
        ...
        [HttpPost]
        public async Task<ActionResult<Respuesta<SituacionEspDBModel>>> PostSituacionEsp([FromBody] requestSituacionEspModel model)
        {
            ...
            try
            {
                ...
                {
                    "opt","IdSitEsp","Descripcion","IdUser","Activo"
                };
                ...
                string storeSituacionEsp = $"Configuracion.SP_SituacionEsp {string.Join(", ", parameterNames.Select(n => "@" + n))}";
                //4.- Almacenamos la información obtenida del store procedure
                List<SituacionEspDBModel> producto = await _dbContext.SituacionEspDB.FromSqlRaw(storeSituacionEsp, parameters).ToListAsync();
                
                if (producto != null && producto.Count > 0)
                {
                    return Ok(new Respuesta<List<SituacionEspDBModel>>{ Exito = 1, Mensaje = "Success",Data = producto });
                }
                else
                {
        ...
        // PUT: api/SituacionEsp/{id}        
        [HttpPut("SituacionEsp")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(Respuesta<SituacionEspDBModel>))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(Respuesta<ErrorTxA>))]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(Respuesta<ErrorTxA>))]
        public async Task<IActionResult> PutSituacionEsp([FromBody] requestSituacionEspModel model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(new Respuesta<ErrorTxA> { Exito = 0, Mensaje = "Invalid Model State", Data = new ErrorTxA { codigo = "01", Mensaje = "Model state validation failed" } });
        ...
        [HttpPut("EstSituacionEsp")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(Respuesta<SituacionEspDBModel>))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(Respuesta<ErrorTxA>))]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(Respuesta<ErrorTxA>))]
        public async Task<IActionResult> PutEstSituacionEsp([FromBody] requestSituacionEspModel model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(new Respuesta<ErrorTxA> { Exito = 0, Mensaje = "Invalid Model State", Data = new ErrorTxA { codigo = "01", Mensaje = "Model state validation failed" } });
        ...
        // GET: api/SituacionEsp
        [HttpGet]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(Respuesta<SituacionEspDBModel>))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(Respuesta<ErrorTxA>))]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(Respuesta<ErrorTxA>))]
        public async Task<ActionResult<IEnumerable<SituacionEspDBModel>>> GetSituacionEspModel()
        {
            try
            {
                //1.- Definimos los parametros que entrarán al store procedure
        ...
        // DELETE: api/SituacionEsp/{id}
        [HttpDelete]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(Respuesta<SituacionEspDBModel>))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(Respuesta<ErrorTxA>))]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(Respuesta<ErrorTxA>))]
        public async Task<IActionResult> DeleteSituacionEsp(string IdSitEsp)
        {
            try
            {                
                //1.- Definimos los parametros que entrarán al store procedure
                ...
                var model = new requestSituacionEspModel { 
                    IdSitEsp = IdSitEsp,
