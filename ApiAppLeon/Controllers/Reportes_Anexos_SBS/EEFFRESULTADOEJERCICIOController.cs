...
// Developer    : VicVil  
// DateCreate   : 17/06/2025
// Description  : Controlador para Generar EEFF RESULTADO EJERCICIO
...
namespace Reportes_Anexos_SBS.Controllers
{
    ...
    [ApiController]
    public class EEFFRESULTADOEJERCICIOController : ControllerBase
    {
        private readonly DBContext _dbContext;

        public EEFFRESULTADOEJERCICIOController(DBContext dbContext)
        {
            _dbContext = dbContext;
        }

        // POST: api/EEFFRESULTADOEJERCICIO
        ...
        [HttpPost]
        public async Task<IActionResult> PostEEFFRESULTADOEJERCICIO([FromBody] requestEEFFRESULTADOEJERCICIOModel model)
        {
            ...
            try
            {
                ...
                string storeEEFFRESULTADOEJERCICIO = $"REPORTES_ANEXOS_SBS.usp_EEFF_ResultadoEjercicio {string.Join(", ", parameterNames.Select(n => "@" + n))}";
                List<EEFFRESULTADOEJERCICIODBModel> producto = await _dbContext.EEFFRESULTADOEJERCICIODB.FromSqlRaw(storeEEFFRESULTADOEJERCICIO, parameters).ToListAsync();
                ...
                var pathPlantilla = Path.Combine(Directory.GetCurrentDirectory(), "Migrations", "recursos\\EEFF\\ResultadoEjercicio", "ResultadoEjercicio.xlsx");
                ...
                var worksheet = package.Workbook.Worksheets.Add("EEFF-ResultadoEjercicio");
                ...
                if (producto != null)
                {
                    ...
                    foreach (EEFFRESULTADOEJERCICIODBModel item in producto)
                    {
                ...
                var fileName = "EEFF - Resultado Ejercicio.xlsx";
        ...
        // GET: api/EEFFRESULTADOEJERCICIO
        [HttpGet]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(FileContentResult))]
        [ProducesResponseType(StatusCodes.Status400BadRequest, Type = typeof(Respuesta<ErrorTxA>))]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(Respuesta<ErrorTxA>))]
        public async Task<ActionResult> GetEEFFRESULTADOEJERCICIOModel([FromQuery] requestCodSucaveDBModel model, [FromQuery] string anio, [FromQuery] string mes)
        {
            if (!ModelState.IsValid)
            {
            ...
            try
            {
                ...
                var parameters = parameterNames.Select(name => new SqlParameter("@" + name, model.GetType().GetProperty(name)?.GetValue(model) ?? DBNull.Value)).ToArray();
                string storeEEFFRESULTADOEJERCICIO = $"REPORTES_ANEXOS_SBS.usp_EEFF_ResultadoEjercicio_Sucave {string.Join(", ", parameterNames.Select(n => "@" + n))}";
                List<SucaveDBModel> producto = await _dbContext.SucaveDB.FromSqlRaw(storeEEFFRESULTADOEJERCICIO, parameters).ToListAsync();
                ...
                string nameAnexo = "03" + anio.Substring(2, 2) + mes + Utilitarios.DaysInMonth(Convert.ToInt32(anio), Convert.ToInt32(mes)).ToString() + "100001202.SEI";
